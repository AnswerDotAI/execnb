...
Jupyter server (or even having it installed). The API is async, so
`await` the run from a notebook or async code, or wrap it in
`asyncio.run` at a sync entry point:
